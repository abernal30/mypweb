...
su<-summary(model)
n<-5
y1<-as.data.frame(tail(y)[n])[,1]
y2<-rownames(as.data.frame(tail(y)[n]))

n<-6
y3<-as.data.frame(tail(y)[n])[,1]
y4<-rownames(as.data.frame(tail(y)[n]))
y5<-as.Date(y4)+1

y6<-su$coefficients[1] + su$coefficients[2] *y3 + su$coefficients[3]*y1
```

Also, suppose we found the following result of the regression:
$$bnb_{t}=\ `r su$coefficients[1]`\ +\ `r su$coefficients[2]`\ bnb_{t-1}\ `r su$coefficients[3]`\ bnb_{t-2}$$


In this case, the forecast of the `r y5` would be:

`r su$coefficients[1]` + `r su$coefficients[2]` * `r y3`+ `r su$coefficients[3]`*`r y1` = `r y6`
